...
//	/**added by zlc 
//	 * ��������һ��Բ��ͼ������Ȧ��һ��ͼ���Բ������*/
//	public void draw(TextureRegion region,float x,float y,float radius){
//		float[] vertices = this.vertices;
...
//		float color = this.color;
//		int idx = this.idx;
//		float prex=(float) (radius * Math.cos(0));
//		float prey=(float) (radius * Math.sin(0));
//		for (int i = 1; i <= cnt; i++) {
//			float angle = (float) ((float)i/ cnt * Math.PI * 2);
//			float bx = (float) (radius * Math.cos(angle));
//			float by = (float) (radius * Math.sin(angle));
//			float x+bx;
//			final float fx2 = x + width;
//			final float fy2 = y + height;
//			final float u = 0;
...
//	}
	...
	//added by zlc for Flash
	public void draw (TextureRegion region, float fx1, float fy1, float fx2, float fy2,float fx3, float fy3,float fx4, float fy4) {
		if (!drawing) throw new IllegalStateException("SpriteBatch.begin must be called before draw.");
